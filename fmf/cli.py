...
class Parser(object):
    ...
    def __init__(self, arguments=None, path=None):
        ...
        if isinstance(arguments, type("")):  # pragma: no cover
            try:
                ...
                self.arguments = [
                    arg.decode("utf8") for arg in shlex.split(arguments.encode("utf8"))
                ]
        ...
        if not hasattr(self, "command_" + self.command):
            ...
            raise utils.GeneralError("Unrecognized command: '{0}'".format(self.command))
    ...
    def clean(self):
        ...
        try:
            ...
            utils.log.error("Unable to remove cache, exception was: {0}".format(error))
